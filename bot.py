...
# ========== ПОКАЗАТЬ ВСЕ ТОВАРЫ (только названия) ==========
...
# ========== ПОКАЗАТЬ КОНКРЕТНЫЙ ТОВАР (с ценой и кнопкой оплаты) ==========
async def show_product(update: Update, context: ContextTypes.DEFAULT_TYPE, product_id: str):
    ...
    text = (
        ...
        f"📝 {description}\n\n"
        f"💰 Цена: **{price} звёзд**\n\n"
        f"⭐ Оплата проходит через Telegram Stars\n"
...
# ========== ОПЛАТА ==========
async def buy_product(update: Update, context: ContextTypes.DEFAULT_TYPE, product_id: str):
    query = update.callback_query
        ...
        if len(parts) >= 2:
            ...
            if link:
                await update.message.reply_text(
                    ...
                    f"🔗 Твоя персональная ссылка (только для тебя):\n{link}\n\n"
                    f"⬆️ Нажми и присоединяйся к каналу.\n\n"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(lambda u, c: show_product(u, c, u.callback_query.data.split("_")[1]), pattern="^product_"))
    app.add_handler(CallbackQueryHandler(lambda u, c: buy_product(u, c, u.callback_query.data.split("_")[1]), pattern="^buy_"))
