...
async def create_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем формат: /create tovar 300 Название
    if len(context.args) < 2:
        await update.message.reply_text(
            "❌ **Использование:** `/create tovar 250 Название товара`\n\n"
            "📝 **Пример:** `/create tovar 300 Бравл Пасс`\n"
            "📝 **Пример:** `/create tovar 500 Премиум VPN`",
            parse_mode="Markdown"
        )
        return
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"💰 Цена: {price}⭐\n"
            f"📦 Название: {name}\n\n"
            f"🗑️ Удалить: `/delete tovar {product_id}`\n"
            f"✏️ Редактировать: `/edit tovar {product_id}`",
            parse_mode="Markdown"
        )
    except ValueError:
        await update.message.reply_text(
...
async def delete_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Использование: `/delete tovar ID`\n\nПример: `/delete tovar 1700000000`")
        return
...
async def edit_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("❌ Использование: `/edit tovar ID`\n\nПример: `/edit tovar 1700000000`")
        return
...
def main():
    ...
    app.add_handler(CommandHandler("create tovar", create_product))
    app.add_handler(CommandHandler("delete tovar", delete_product))
    app.add_handler(CommandHandler("edit tovar", edit_product))
